...
we started this report by just focusing on getting a good understanding of the dataset. The EDA is detailed and many visualizations are included, and also includes modeling. 
...
there is a brief introduction in the main page of the dataset in kaggle, so we put them there:

Ask a home buyer to describe their dream house, and they probably won't begin with the height of the basement ceiling or the proximity to an east-west railroad. But this playground competition's dataset proves that much more influences price negotiations than the number of bedrooms or a white-picket fence.

With 79 explanatory variables describing (almost) every aspect of residential homes in Ames, Iowa, this competition challenges you to predict the final price of each home.

although this is a competition about predicting the house prices, it is useful to do some other data analysis such as EDA, with this special dataset we can improve our modeling skills and the ability to fully understand data.
...
The train dataset consist of character and integer variables. Most of the character variables are actually (ordinal) factors, but we chose to read them into R as character strings as most of them require cleaning and/or feature engineering first. In total, there are 81 columns/variables, of which the last one is the response variable (SalePrice). Below, we are displaying only a glimpse of the variables. All of them are discussed in more detail throughout the document.
...
As you can see, the sale prices are right skewed. This was expected as few people can afford very expensive houses. We will keep this in mind, and take measures before modeling.
...
The character variables need some work before can use them. To get a feel for the dataset, we can find which numeric variables have a high correlation with the SalePrice.
...
In the remainder of this section, we will visualize the relation between SalePrice and the two predictors with the highest correlation with SalePrice; Overall Quality and the 'Above Grade' Living Area
...
First of all, we need to see which variables contain missing values.
...
In this section, we am going to fix the 34 predictors that contains missing values. As we want to keep the document as readable as possible, we decided to use the "Tabs" option that knitr provides. You can find a short analysis for each (group of) variables under each Tab. You don't have to go through all sections, and can also just have a look at a few tabs.

Besides making sure that the NAs are taken care off, we have also converted character variables into ordinal integers if there is clear ordinality, or into factors if levels are categories without ordinality. we will convert these factors into numeric later on by using one-hot encoding (using the model.matrix function).
...
Now, we can use the function 'revalue' to do the work.
...
However, there is a second variable that relates to Pools. This is the PoolArea variable (in square feet). As you can see below, there are 3 houses without PoolQC. First, we checked if there was a clear relation between the PoolArea and the PoolQC. As we did not see a clear relation (bigger of smaller pools with better PoolQC), we am going to impute PoolQC values based on the Overall Quality of the houses (which is not very high for those 3 houses).
...
Within Alley, there are 2721 NAs. As the values are not ordinal, we will convert Alley into a factor. Values:
...
Our conclusion is that the values do not seem ordinal (no fence is best). Therefore, we will convert Fence into a factor.
...
The number of NAs in FireplaceQu matches the number of houses with 0 fireplaces. This means that we can safely replace the NAs in FireplaceQu with 'no fireplace'. The values are ordinal, and we can use the Qualities vector that we have already created for the Pool Quality. Values:
...
First of all, we are going to replace all 159 missing **GarageYrBlt: Year garage was built** values with the values in YearBuilt.

```{r}
all$GarageYrBlt[is.na(all$GarageYrBlt)] <- all$YearBuilt[is.na(all$GarageYrBlt)]
```

As NAs mean 'No Garage' for character variables, we now want to find out where the differences between the 157 NA GarageType and the other 3 character variables with 159 NAs come from.
...
The 157 NAs within GarageType all turn out to be NA in GarageCondition, GarageQuality, and GarageFinish as well. The differences are found in houses 2127 and 2577. As you can see, house 2127 actually does seem to have a Garage and house 2577 does not. Therefore, there should be 158 houses without a Garage. To fix house 2127, we will imputate the most common values (modes) for GarageCond, GarageQual, and GarageFinish.
...
Now, the 4 character variables related to garage all have the same set of 158 NAs, which correspond to 'No Garage'. we will fix all of them in the remainder of this section

**GarageType: Garage location**

The values do not seem ordinal, so we will convert into a factor.
...
So altogether, it seems as if there are 79 houses without a basement, because the basement variables of the other houses with missing values are all 80% complete (missing 1 out of 5 values). We are going to impute the modes to fix those 9 houses.
...
Now that the 5 variables considered agree upon 79 houses with 'no basement', We are going to factorize/hot encode them below.
...
We now still have to deal with those 6 variables that have 1 or 2 NAs.
...
It should be obvious that those remaining NAs all refer to 'not present'. Below, We are fixing those remaining variables.
...
Masonry veneer type has 24 NAs. Masonry veneer area has 23 NAs. If a house has a veneer area, it should also have a masonry veneer type.
...
There seems to be a significant difference between "common brick/none" and the other types. We assume that simple stones and for instance wooden houses are just cheaper. We will make the ordinality accordingly.
...
However, the table below shows that only one house does not have all public utilities. This house is in the train set. Therefore, imputing 'AllPub' for the NAs means that all houses in the test set will have 'AllPub'. This makes the variable useless for prediction.
...
Note: as the number of levels is really high, We will look into binning later on.
...
We have now finally reached the point where all character variables have been converted into categorical factors or have been label encoded into numbers. In addition, 3 numeric variables have been converted into factors, and we deleted one variable (Utilities). As you can see below, the number of numerical variables is now 56 (including the response variable), and the remaining 23 variables are categorical.
...
Below we are checking the correlations again. As you can see, the number of variables with a correlation of at least 0.5 with the SalePrice has increased from 10 (see section 4.2.1) to 16. 
...
Although the correlations are giving a good overview of the most important numeric variables and multicolinerity among those variables, we wanted to get an overview of the most important variables including the categorical variables before moving on to visualization.

We tried to get the relative importance of variables with a quick linear regression model with the calc.relimp function of package , and also tried the boruta function of package boruta which separates the variables into groups that are important or not. However, these method took a long time. As we only want to get an indication of the variable importance, we eventually decided to keep it simple and just use a quick and dirty Random Forest model with only 100 trees. This also does the job for me, and does not take very long as we can specify a (relatively) small number of trees.
...
we have already visualized the relation between Overall Quality and SalePrice in my initial explorations, but we want to visualize the frequency distribution as well. As there are more quality measurements, we are taking the opportunity to bundle them in this section.
...
Overall Quality is very important, and also more granular than the other variables. External Quality is also improtant, but has a high correlation with Overall Quality (0.73). Kitchen Quality also seems one to keep, as all houses have a kitchen and there is a variance with some substance. Garage Quality does not seem to distinguish much, as the majority of garages have Q3. Fireplace Quality is in the list of high correlations, and in the important variables list. The PoolQC is just very sparse (the 13 pools cannot even be seen on this scale). We will look at creating a 'has pool' variable later on.
...
Several Garage variables have a high correlation with SalePrice, and are also in the top-20 list of the quick random forest. However, there is multicolinearity among them and 7 garage variables is too many anyway.
...
So it seemed as if the Total Basement Surface in square feet (TotalBsmtSF) is further broken down into finished areas (2 if more than one type of finish), and unfinished area. we did a check between the correlation of total of those 3 variables, and TotalBsmtSF. The correlation is exactely 1, so that's a good thing (no errors or small discrepancies)!
...
There are 4 bathroom variables. Individually, these variables are not very important. However, we assume that if add them up into one predictor, this predictor is likely to become a strong one.
...
Finally, we are creating the IsNew variable below. Altogether, there are 116 new houses in the dataset.
...
Below, we listed the variables that seem related regarding porches.
...
As far as we know, porches are sheltered areas outside of the house, and a wooden deck is unsheltered. Therefore, we are leaving WoodDeckSF alone, and are only consolidating the 4 porch variables.
...
First of all, we are dropping a variable if two variables are highly correlated. To find these correlated pairs, we have used the correlations matrix again (see section 6.1). For instance: GarageCars and GarageArea have a correlation of 0.89. Of those two, we are dropping the variable with the lowest correlation with SalePrice (which is GarageArea with a SalePrice correlation of 0.62. GarageCars has a SalePrice correlation of 0.64).
...
For the time being, we are keeping it simple and just remove the two really big houses with low SalePrice manually. However, we intend to investigate this more thorough in a later stage (possibly using the 'outliers' package).
...
In previous versions, we worked with Caret's `Near Zero Variance` function. Although this works, it also is a quick fix and too much information got lost. For instance, by using the defaults, all Neighborhoods with less than 146 houses are omitted as (one-hot encoded) variables (frequency ratio higher than 95/5). Therefore, we have taken a more carefully manual approach in this version.
...
Altogether, we have removed 49 one-hot encoded predictors with little or no variance. Altough this may seem a significant number, it is actually much less than the number of predictors that were taken out by using caret's`near zero variance` function (using its default thresholds).
...
The skew of 1.87 indicates a right skew that is too high, and the Q-Q plot shows that sale prices are also not normally distributed. To fix this we are taking the log of SalePrice.
...
write.csv(train1, file = 'processend_data_train.csv', row.names = F)
write.csv(test1, file = 'processend_data_test.csv', row.names = F)
...
we have also tried Ridge and Elastic Net models, but since lasso gives the best results of those 3 models I am only keeping the lasso model in the document.
...
Below, we are using caret cross validation to find the best value for lambda, which is the only hyperparameter that needs to be tuned for the lasso model.
...
Initially, we just worked with the XGBoost package directly. The main reason for this was that the package uses its own efficient datastructure (xgb.DMatrix). The package also provides a cross validation function. However, this CV function only determines the optimal number of rounds, and does not support a full grid search of hyperparameters.

Although caret does not seem to use the (fast) datastructure of the xgb package, we eventually decided to do hyperparameter tuning with it anyway, as it at least supports a full grid search. As far as we understand it, the main parameters to tune to avoid overfitting are max_depth, and min_child_weight (see [XGBoost documentation](http://xgboost.readthedocs.io/en/latest/how_to/param_tuning.html)). Below we are setting up a grid that tunes both these parameters, and also the eta (learning rate).
...
As expected, this took quite a bit of time (locally). As we want to limit the running time, we disabled the code, and are just continuing with the results. According to caret, the 'bestTune' parameters are:

* Max_depth=3
* eta=0.05
* Min_child_weight=4

In the remainder of this section, we will continue to work with the xgboost package directly. Below, we are starting with the preparation of the data in the recommended format.
...
In addition, we are taking over the best tuned values from the caret cross validation.
...
Since the lasso and XGBoost algorithms are very different, averaging predictions likely improves the scores. As the lasso model does better regarding the cross validated RMSE score (0.1121 versus 0.1162), we are weigting the lasso model double.
